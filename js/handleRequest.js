...
function onTextClick() {
		...
		function createnewBar(score) {
			return $(`<div id=\"bar\" class=\"segments load\"><div class=\"from-watson top\"><div class=\"message-inner\"><div class=\"column\"><progress class=\"progress progress-good is-medium\" value=\"${score}\" max=\"100\">${score}</progress>${score}</div></div></div></div>`);			
		}
		...
		function processOK(response) {
			...
			if ('undefined' !== typeof response.intents[0].confidence)
			    the_score = response.intents[0].confidence*100;
				//the_score = (Math.round((response.intents[0].confidence*1000)/10)/100).toFixed(2);
			$('#id_contextdump').prepend(createnewBar(the_score.toFixed(3)));
        	$('#id_contextdump').show();
